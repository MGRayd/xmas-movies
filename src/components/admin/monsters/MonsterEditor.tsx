...
import BasicFields from "@/components/admin/monsters/BasicFields";
...
import MonsterRelations from "@/components/admin/monsters/MonsterRelations";
import DeceasedToggle from "@/components/admin/monsters/DeceasedToggle";
import { Option } from "@/components/RelationPicker";
import { useToast } from "@/ui/ToastProvider";

type AnyMap = Record<string, any>;

export default function MonsterEditor({ mode }: { mode: "create" | "edit" }) {
  const { id } = useParams();
  const col = "monsters";
  ...
  const [form, setForm] = useState<AnyMap>({
    ...
    notableDetails: "",
    knownEncounters: "",
    firstAppearance: null as string | null,
    appearanceMarkdown: "",
    detailsMarkdown: "",
    encountersMarkdown: "",
    published: true,
  ...
  async function save() {
    ...
    const basePayload: AnyMap = {
      ...
      encountersMarkdown: form.encountersMarkdown || "",
    };
    ...
    try {
      ...
      console.error(e);
      // ❌ error toast
      toast.error(e?.message || "Failed to save monster.");
    }
  }
  ...
  return (
    <div className="container mx-auto max-w-3xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        <Collapsible title="Notable Details" storageKey="sec-details" defaultOpen>
          <MarkdownEditor
            value={form.detailsMarkdown || ""}
            onChange={(v) => setForm((s) => ({ ...s, detailsMarkdown: v }))}
          />
        </Collapsible>

        <Collapsible title="Known Encounters" storageKey="sec-encounters" defaultOpen>
          <MarkdownEditor
            value={form.encountersMarkdown || ""}
            onChange={(v) => setForm((s) => ({ ...s, encountersMarkdown: v }))}
          />
        </Collapsible>
        ...
        <Collapsible title="Status" storageKey="sec-status">
          <div className="space-y-4">
            ...
            <DeceasedToggle
              ...
              onChange={(v) => setForm((s) => ({ ...s, deceased: v }))}
            />
          </div>
        </Collapsible>
